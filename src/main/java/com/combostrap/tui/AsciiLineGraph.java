...
public class AsciiLineGraph {
  ...
  private AsciiLineGraph(double[] series) {
    this.series = series;
  }
  ...
  private void calculateFields() {
    ...
    this.min = Arrays.stream(this.series).min().orElseThrow(()->new RuntimeException("No min found"));
    this.max = Arrays.stream(this.series).max().orElseThrow(()->new RuntimeException("No max found"));
    this.range = max - min;
   ...
   */
  public static AsciiLineGraph fromSeries(double[] series) {
    return new AsciiLineGraph(series);
  }
   ...
   */
  public AsciiLineGraph withNumRows(int numRows) {
   ...
   * @return This instance.
   */
  public AsciiLineGraph withTickWidth(int tickWidth) {
   ...
   * @return This instance.
   */
  public AsciiLineGraph withTickFormat(DecimalFormat tickFormat) {
   ...
   * in order to be {@link AsciiLineGraph#tickWidth} characters long.
   *
